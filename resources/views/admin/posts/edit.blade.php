                    ...
                    <form action="{{ route('admin.posts.update', $post->id) }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('tieu_de') is-invalid @enderror" 
                                   id="tieu_de" name="tieu_de" value="{{ old('tieu_de', $post->tieu_de) }}" required>
                            @error('tieu_de')
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('noi_dung') is-invalid @enderror" 
                                      id="noi_dung" name="noi_dung" rows="3">{{ old('noi_dung', $post->noi_dung) }}</textarea>
                            @error('noi_dung')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-control @error('trang_thai') is-invalid @enderror" 
                                ...
                                <option value="1" {{ old('trang_thai', $post->trang_thai) == '1' ? 'selected' : '' }}>
                                    Hoạt động
                                </option>
                                <option value="0" {{ old('trang_thai', $post->trang_thai) == '0' ? 'selected' : '' }}>
                                    Không hoạt động
                                </option>
                            </select>
                            @error('trang_thai')
