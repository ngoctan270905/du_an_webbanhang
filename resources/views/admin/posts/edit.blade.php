...
@section('content')
    <div class="container mx-auto flex items-center justify-center py-8">
        <div class="w-full max-w-xl rounded-xl bg-white p-6 shadow-2xl transition-all duration-300 hover:shadow-3xl sm:p-8">
            <div class="mb-8 text-center">
                <h3 class="text-3xl font-bold text-indigo-900">Chỉnh sửa bài viết</h3>
                <p class="mt-2 text-gray-500">Cập nhật thông tin chi tiết cho bài viết.</p>
            </div>
            ...
            <form action="{{ route('admin.posts.update', $post->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" id="tieu_de" name="tieu_de" value="{{ old('tieu_de', $post->tieu_de) }}" required
                ...
                <div>
                    ...
                    <textarea id="noi_dung" name="noi_dung" rows="6"
                        ...
                        placeholder="Nhập nội dung bài viết...">{!! old('noi_dung', $post->noi_dung) !!}</textarea>
                    @error('noi_dung')
                        <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <div id="image-preview-container"
                        class="mt-4 overflow-hidden rounded-lg border border-dashed border-gray-300 bg-gray-100 p-2 text-center transition-all duration-300">
                        <img id="image-preview" src="{{ asset('storage/' . $post->hinh_anh) }}" alt="Image Preview" class="mx-auto block h-48 w-full rounded-lg object-contain">
                ...
                <div>
                    ...
                    <select id="trang_thai" name="trang_thai" required
                        ...
                        <option value="1" {{ old('trang_thai', $post->trang_thai) == 1 ? 'selected' : '' }}>Hoạt động</option>
                        <option value="0" {{ old('trang_thai', $post->trang_thai) == 0 ? 'selected' : '' }}>Không hoạt động</option>
                    </select>
                    @error('trang_thai')
                        <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                    @enderror
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            // Initial load check for existing image
            updatePreview(imagePreview.src);
            ...
            fileInput.addEventListener('change', function () {
                ...
                if (file) {
                    ...
                    // Revert to existing image if user cancels file selection
                    updatePreview("{{ asset('storage/' . $post->hinh_anh) }}");
                }
            });
        });
    </script>
