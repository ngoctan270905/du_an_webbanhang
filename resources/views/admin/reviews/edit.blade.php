        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">Chỉnh sửa đánh giá</h3>
                </div>
                <div class="card-body">
                    <form action="{{ route('admin.reviews.update', $review->id) }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-control @error('id_nguoi_dung') is-invalid @enderror" 
                                ...
                                @foreach($users as $user)
                                    <option value="{{ $user->id }}" {{ old('id_nguoi_dung', $review->id_nguoi_dung) == $user->id ? 'selected' : '' }}>
                                        {{ $user->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-control @error('id_san_pham') is-invalid @enderror" 
                                ...
                                @foreach($products as $product)
                                    <option value="{{ $product->id }}" {{ old('id_san_pham', $review->id_san_pham) == $product->id ? 'selected' : '' }}>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('noi_dung') is-invalid @enderror" 
                                      id="noi_dung" name="noi_dung" rows="3">{{ old('noi_dung', $review->noi_dung) }}</textarea>
                            @error('noi_dung')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-control @error('trang_thai') is-invalid @enderror" 
                                ...
                                <option value="1" {{ old('trang_thai', $review->trang_thai) == '1' ? 'selected' : '' }}>
                                    Hoạt động
                                </option>
                                <option value="0" {{ old('trang_thai', $review->trang_thai) == '0' ? 'selected' : '' }}>
                                    Không hoạt động
                                </option>
                            </select>
                            @error('trang_thai')
