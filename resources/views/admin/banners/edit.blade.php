...
<h3 class="text-3xl font-bold text-indigo-900">Chỉnh Sửa Banner</h3>
<p class="mt-2 text-gray-500">Cập nhật thông tin chi tiết của banner.</p>
</div>
        ...
        <form action="{{ route('admin.banners.update', $banner->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="mb-6">
                ...
                <input type="text" id="tieu_de" name="tieu_de" value="{{ old('tieu_de', $banner->tieu_de) }}" required
            ...
            <div class="mb-6">
                ...
                <div class="relative">
                    <label for="hinh_anh"
                        ...
                        <span class="ml-2">Chọn tệp ảnh mới</span>
                        <input type="file" id="hinh_anh" name="hinh_anh"
                            class="sr-only @error('hinh_anh') border-red-500 @enderror">
                    </label>
                </div>
                <p class="mt-1 text-xs text-gray-500">Chỉ chấp nhận file ảnh (jpeg, png, jpg, gif) và kích thước tối đa 2MB. Bỏ trống nếu không muốn thay đổi.</p>
                @error('hinh_anh')
                    <div class="mt-1 text-sm text-red-500">{{ $message }}</div>
                @enderror
            </div>
            ...
            <!-- Image preview -->
            <div id="image-preview" class="mb-6 {{ $banner->hinh_anh ? '' : 'hidden' }}">
                <label class="mb-2 block text-sm font-medium text-indigo-900">Xem trước hình ảnh</label>
                <img id="preview-image-tag" src="{{ $banner->hinh_anh ? asset('storage/' . $banner->hinh_anh) : '' }}" alt="Image Preview"
            ...
            <div class="mb-6">
                ...
                <div class="relative">
                    <select id="trang_thai" name="trang_thai" required
                        ...
                        <option value="1" {{ old('trang_thai', $banner->trang_thai) == '1' ? 'selected' : '' }}>Hoạt động</option>
                        <option value="0" {{ old('trang_thai', $banner->trang_thai) == '0' ? 'selected' : '' }}>Không hoạt động</option>
                    </select>
                </div>
                @error('trang_thai')
                    <div class="mt-1 text-sm text-red-500">{{ $message }}</div>
