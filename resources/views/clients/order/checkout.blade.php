...
@section('content')
    ...
    <div class="container mx-auto p-4 md:p-8 lg:p-12 bg-gradient-to-br from-blue-50 to-indigo-100 font-sans antialiased text-gray-800">
        ...
        <form id="order-form" action="{{ route('order.create') }}" method="POST">
            @csrf
            <input type="hidden" name="phi_van_chuyen" id="shipping-cost-hidden" value="30000">
                    ...
                    <div class="bg-white p-8 rounded-xl shadow-lg border border-blue-100">
                        ...
                        <div class="flex flex-col gap-4">
                            ...
                            <label
                                ...
                                <div
                                    class="relative w-5 h-5 border border-gray-400 rounded-full flex items-center justify-center radio-indicator">
                                    <input type="radio" name="phuong_thuc_thanh_toan" value="online_payment"
                                           class="absolute opacity-1 w-full h-full cursor-pointer">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3 text-white hidden"
                                         viewBox="0 0 20 20" fill="currentColor">
                                        <path fill-rule="evenodd"
                ...
                <div class="lg:w-1/3">
                    <div class="bg-white p-8 rounded-xl shadow-lg border border-blue-100 sticky top-12">
                        ...
                        <div class="space-y-4 text-gray-700">
                            ...
                            <div class="flex justify-between items-center font-extrabold text-2xl border-t border-blue-200 pt-6 mt-6">
                                <span>Tổng cộng</span>
                                <span class="text-blue-600" id="total-price">{{ number_format($total + 30000, 0, ',', '.') }}đ</span>
                            </div>
                        </div>
                        ...
                        <p class="text-center text-xs text-gray-500 mt-4">Bằng cách nhấp vào "Xác Nhận Thanh Toán Ngay", bạn đồng ý với các Điều Khoản Dịch Vụ.</p>
                    </div>
                </div>
            </div>
        </form>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            shippingRadios.forEach((radio) => {
                radio.addEventListener('change', () => {
                    ...
                    totalPriceDisplay.textContent = formatCurrency(baseTotal + shippingCost);
            ...
            const handleSubmitOrder = debounce(function () {
                ...
                submitOrderBtn.innerHTML = '<i class="fas fa-spinner text-xl mr-3 align-middle"></i> Đang xử lý...';
                ...
                .then(data => {
                    ...
                    submitOrderBtn.innerHTML = '<i class="fas fa-check-circle text-xl mr-3 align-middle"></i> Xác Nhận Thanh Toán Ngay';
            ...
            confirmChangesBtn.addEventListener('click', function () {
                ...
                fetch('{{ route('cart.apply_changes') }}', {
                    ...
                    body: JSON.stringify({ updated_items: updatedItems })
                })
                .then(response => {
                    if (!response.ok) throw new Error(`HTTP error! Status: ${response.status}`);
                    return response.json();
